    ...
    <el-card shadow="never">
      <template #header>
        <div class="card-header">
          <span>故障工单</span>
          <el-button type="danger" size="small" @click="$router.push('/workorder/create')">创建故障单</el-button>
        </div>
      </template>
      ...
      <el-table :data="data" stripe v-loading="loading">
        ...
        <el-table-column prop="title" label="故障描述" min-width="160" />
...
const { loading, data, pagination, fetchData, handlePageChange, handleSearch, handleReset } = useTable(
  (params) => workorderStore.fetchList({ ...params, type: 2 }),
